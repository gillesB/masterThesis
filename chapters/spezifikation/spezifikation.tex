...
Bei der Spezifikation werden möglichst nur allgemein bekannte technische Begriffe verwendet, die wenigen spezifischeren Ausnahmen werden hier erklärt. \todo{ref zum technischen Umfeld?; Gehört das hierhin? Abschnitt mit dem Cids-Umfeld vor die Spezifikation ziehen? Dies bedeutet allerdings einen Bruch der Sicht. Kapitel 2,3 fachliche Sicht, Kapitel 4 technische Sicht.}
\begin{description}
...
\subsection{Kunde}
Im aktuellen Stand werden die Kunden des Ressorts 102 nicht in \ac{WuNDa} verwaltet, durch die gewünschten Erweiterungen wird dies jedoch zwingend notwendig. Daher wird eine Klasse  benötigt die es ermöglicht Kunden zu verwalten und darzustellen. 
...
Weiterhin soll die jeweils letzte Änderung protokolliert werden. Auf eine Historie mit den gemachten Änderungen wird bewusst verzichtet, erfolgen also zwei Änderungen so wird das Protokoll der ersten Änderung überschrieben.
...
Der Bericht wurde allerdings nicht vollständig vom R102 spezifiziert und kann somit auch nicht komplett umgesetzt werden.
Aus diesem Grund soll der Bericht lediglich in den Masken vorgesehen werden und ein Beispielbericht soll auch generiert werden können.
Dieser Bericht entspricht allerdings nicht der tatsächlichen Geschäftsstatistik. 
