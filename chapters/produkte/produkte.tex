...
\ac{WuNDa} unterstützt die externen Benutzer bei der Selbstentnahme der Produkte die im Abschnitt \ref{sec:produktliste} aufgelistet sind.
...
Weitere Eingabefelder sind das Format (z.B. DINA4), den  Maßstab (hier 1:500), die Auftragsnummer und ein Kommentarfeld. Die Option "`Drehwinkel vorschlagen"' beeinflusst den später angezeigten Druckbereich. \todo{Bild neu machen}
...
Im Downloadprotokoll muss der externe Benutzer eine Geschäftsbuchnummer angeben, die Angabe der Projektbezeichnung ist allerdings optional.
Weiterhin ist die Angabe des Verwendungszweckes interessant, für einen \ac{ÖbVI} stehen hier z.B. "`Vermessungsunterlagen (amtlicher Lageplan TS3)"' und "`Vermessungsunterlagen (hoheitliche Vermessung TS4)"' zur Verfügung.
In dem Berechnungsfeld sieht der Benutzer wie hoch die Gebühr für das bezogene Produkt sein wird. In diesem Fall handelt es sich um eine Gebühr von \EUR{0} da es sich bei dem Benutzer um ein \ac{ÖbVI} handelt der ein vermessungsrelevantes Dokument bezieht.
Mit dem Bestätigten des Dialoges akzeptiert der Benutzer die Gebühr und der Download des Produktes wird gestartet. \todo{Abschluss finden. Auch wenn Gebühr = 0. es wird trotzdem protokolliert...}
