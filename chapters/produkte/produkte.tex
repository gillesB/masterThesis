...
\paragraph{Wuppertal}
Im Augenblick ist es bei der Stadt Wuppertal nur möglich die NAS-Dateien zu beziehen. Demnach funktioniert das, oben beschriebene, Zurückgeben der NAS-Erhebungsdaten nicht.
Aus diesem Grund erstellt der \ac{ÖbVI} Dokumente, z.\,B. Fortführungsrisse \todo{Stimmt das? Welche Dokumente noch?} aus seinen ausgewerteten Daten und den ermittelten Punkten und gibt diese an das Katasteramt weiter.
Dort müssen diese manuell von Beamten mittels des 3A Editors eingepflegt werden. Bei dem 3A Editor handelt es sich um eine Anwendung von AED-SICAD, mit der es möglich ist ALKIS-Daten fortzuführen \autocite[vgl.][]{sicad-3a}.
