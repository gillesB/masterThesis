...
Die Produkte die diese Bedingungen erfüllen werden im folgenden vermessungsrelevant genannt. Der Rabatt der bei diesen Produkten gegeben wird, wird "`zweckabhängiger Rabatt"' genannt. \todo{evt. noch MwSt erwähnen. Rabett der Selbstentnahme erwähnen.}
...
Der zweite Schritt der Auswertung besteht im Einlesen der Rohmessdaten in das Berechnungsprogramm \todo{oder ins CAD? oder beides?}. Dieses kann den benötigten Fortführungsriss und die Skizze zur Grenzniederschrift automatisch erzeugen.
...
\todo{STIMMT SO NICHT. Siehe pdf mit den Anmerkungen von Frah Hähner.}
...
Ungeachtet dessen bereichert dieser Zugriff die Arbeit der Polizeibeamten, da die benötigten Informationen auch außerhalb der Öffnungszeiten des Katasteramtes bezogen werden können.
...
Kurzum der externe Benutzer erhält durch die Selbstentnahme die Möglichkeit die benötigten Produkte, von seinem Rechner aus, selbst zu beziehen, so dass diese unmittelbar für ihn zur Verfügung stehen.
Weiterhin entlastet er somit das Katasteramt, weil die Bearbeitungszeit zum Erstellen der Produkte eingespart wird.
Wodurch ein zweiter Vorteil für den Benutzer entsteht, da er einen Rabatt auf den kostenpflichtigen Produkten erhält.
...
Ansonsten wird dem Benutzer in dem Bereich keine Funktionalität angeboten.
...
Im Workflow \ref{subsec:polizei_ermittlung} hat man gesehen, dass die Polizei mehrere externe Benutzer besitzt.
In naher Zukunft wird es voraussichtlich noch andere Kunden geben, die mehrere Benutzer besitzen. \todo{polizei streichen, die zahlen nix}
Die Abrechnung soll allerdings an den eigentlichen Kunden gestellt werden und nicht an die einzelnen Benutzer.
